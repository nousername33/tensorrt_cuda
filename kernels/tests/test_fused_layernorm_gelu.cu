...
static std::vector<float> cpu_ref(const std::vector<float>& x,
  ...
  for (int r = 0; r < rows; ++r) {
    ...
    for (int c = 0; c < cols; ++c)
      y[(size_t)r * cols + c] =
          cpu_gelu((row[c] - mean) * rstd * gamma[c] + beta[c]);
  }
  return y;
}
...
void test_fused_layernorm_gelu() {
  ...
  dgamma.upload(gamma);
  dbeta.upload(beta);
  ck::fused_layernorm_gelu_forward(dx.ptr, dgamma.ptr, dbeta.ptr, dy.ptr, rows,
                                   cols);
  const auto got = dy.download();

  const auto ref = cpu_ref(x, gamma, beta, rows, cols, 1e-5f);
  ktest::close("fused_layernorm_gelu", got, ref, 1e-3f, 1e-4f);
}
